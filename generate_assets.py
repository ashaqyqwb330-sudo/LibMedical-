...
def reshape_text(text):
    """Shape and reverse Arabic text to support right-to-left layout in FPDF2 & PIL rendering."""
    if not text:
        return ""
    try:
        # Standard reshape
        reshaped = arabic_reshaper.reshape(text)
        # Apply bidirectional display algorithm
        bidi_text = get_display(reshaped)
        return bidi_text
    except Exception as e:
        # Fallback to pure reversed text as a last resort in non-supported unicode environments
        return text
...
def build_pdf_clinical_book(pdf_path, title, book_data):
    ...
    # References
    pdf.set_font("Core", "", 10) # Courier/Sans fallback font or Amiri
    pdf.set_font("Amiri", size=11)
    pdf.set_text_color(71, 85, 105)
...
def draw_luxurious_png_cover(cover_path, title):
    ...
    draw.text((cx, 165), reshape_text("كليّة الطب الميداني العسكري الأكاديمي"), fill=(241, 245, 249, 255), font=font_crest, anchor="mm")
    draw.text((cx, 185), reshape_text("مناهج الطوارئ والدفاع الطبي الحربي"), fill=gold_rgb + (255,), font=font_tagline, anchor="mm")
    ...
    for w in words:
        ...
        w_px = draw.textlength(reshape_text(candidate_line), font=font_title)
        if w_px < (width - 70):
            line_cur.append(w)
        else:
            if line_cur:
    ...
    for l in lines:
        ...
        draw.text((cx + 1, starting_y + 1), reshape_text(l), fill=(0, 0, 0, 180), font=font_title, anchor="mm")
        draw.text((cx, starting_y), reshape_text(l), fill=(255, 255, 255, 255), font=font_title, anchor="mm")
        starting_y += 34
    ...
    draw.text((cx, badge_y), reshape_text("🎖️ مقرر رسمي ومكثف - معتمد للدفاع"), fill=gold_rgb + (255,), font=font_tagline, anchor="mm")
    
    # Outer Framing & Signature Labels
    draw.text((cx, height - 70), reshape_text("منشورات الجمعية الطبية العسكرية المشتركة"), fill=(148, 163, 184, 255), font=font_sub, anchor="mm")
    draw.text((cx, height - 45), reshape_text("⚠️ سري وللاستخدام التدريبي فقط"), fill=(231, 76, 60, 255), font=font_crest, anchor="mm")
